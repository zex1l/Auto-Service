...
const getUserBasket = async () => {
  ...
  data  && dispatch(updateCart({count: data[0].goods.length, goods: data[0].goods}))
}
      ...
      <div className={css.header__btn}>
        {user && (
          <Link to='/basket' className={css.header__shopingCart}>
            <Badge  count={count} children={<ShoppingCartOutlined style={{fontSize: 30, color: '#fff'}}/>}></Badge>
          </Link>
        )}
        <AddRecords />
        {user && (
